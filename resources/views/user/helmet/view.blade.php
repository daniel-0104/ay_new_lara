...
<body data-page="page2">
    ...
    <section class="container-lg" id="product-view">
        <a href="javascript:history.back()" id="back-arrow" class="arrow">
            ...
            <form action="">
                <div class="row align-items-center justify-content-center mx-auto pb-4">
                    <div class="col-lg-5 col-md-5 d-flex align-items-center justify-content-center">
                        <img src="{{ asset('storage/'.$helmets->image) }}" class="w-100 d-block rounded">
                    </div>
                    ...
                    <div id="product-details">
                        <div class="mb-2">
                            <label for="">Type of Bicycle Helmet : <span> {{ $helmets->type }} </span></label>
                            <label for="">Color : <span> {{ $helmets->color }} </span></label>
                            <label for="">Price : <span> {{ $helmets->price }} Kyats </span></label>
                            <label for="">Material : <span> {{ $helmets->material }} </span></label>
                            <label for="">Viewers : <span> {{ $helmets->view_count }} </span></label>
                        </div>
                        <p style="font-family: calibri;"> {{ $helmets->description }} </p>
                    </div>
                        <div class="mb-4 pt-2" id="order-count-btn" >
                            <div class="input-group quantity me-3" style="width: 130px;">
                                ...
                                <input type="hidden" value="{{ $helmets->name }}" id="helmetName">
                                <div class="input-group-btn">
    ...
    <section id="related-product" class="my-5 px-5">
        ...
        <div class="swiper mySwiper" id="mySwiper">
            <div class="swiper-wrapper">
                @foreach ($helmetList as $h)
                <div class="swiper-slide">
                        <input type="hidden" class="userId" value="{{ Auth::user()->id }}">
                        <input type="hidden" class="productName" value="{{ $h->name }}">
                        <input type="hidden" class="orderCount" value="1">
                    <div id="product-img-content">
                        <img src="{{ asset('storage/'.$h->image) }}" class="w-100 d-block" style="height: 290px;">
                        <div id="product-img-link">
                            <a href="{{ route('user#helmetView',$h->id) }}" id="eye"><i class="fa-solid fa-eye"></i></a>
                    ...
                    <div id="product-name" class="mt-2 border px-1 py-1 text-center">
                        <h5>{{ $h->name }}</h5>
                        <span style=" color: #a2b2ee;"> {{ $h->price }} Kyats </span>
                    </div>
                </div>
                @endforeach
            </div>
...
<script>
    ...
    // load more data
    var ENDPOINT = "{{ route('user#helmetView',$helmets->id) }}" ;
    var page = 1;
